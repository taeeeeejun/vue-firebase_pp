  ...
  <div>
    ...
    <v-list nav>
      <v-list-group
        ...
        <v-list-item @click="openDialogSubItem(index, -1)" v-if="$store.state.editable">
    ...
    <v-dialog v-model="dialogSubItem" max-width="400">
      <v-card>
        <v-card-title>
          서브 아이템 수정하기
          <v-spacer/>
          <v-btn icon @click="saveSubItem" color=""><v-icon>mdi-content-save</v-icon></v-btn>
...
export default {
  ...
  methods: {
    ...
    async save () {
      try {
        await this.$firebase.database().ref().child('site').set({
          menu: this.items
        })
      } finally {
